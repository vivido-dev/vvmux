...
fn a_detected_agent_reaches_blocked_from_its_screen_alone() {
    ...
    // A session-only hook enriches the screen-classified agent without claiming its state.
    let session_report = json(command(
        &runtime,
        &name,
        &[
            ...
            "24",
            "--agent-session-id",
            "conversation-screen-1",
            "--pane-id",
            &pane,
        ],
    ));
    assert_eq!(session_report["agent"]["source"], "screen");
    assert_eq!(session_report["agent"]["session_present"], true);
    let explained = json(command(
        &runtime,
        &name,
        &["agent-explain", "--pane-id", &pane],
    ));
    assert_eq!(explained["explain"]["source"], "screen");
    assert!(explained["explain"]["report"].is_null());
    let inspected = json(command(&runtime, &name, &["inspect", "--pane-id", &pane]));
    assert_eq!(
        inspected["pane"]["agent"]["agent_session"]["id"],
    ...
    let listed = json(command(&runtime, &name, &["list-panes"]));
    let listed_agent = &listed["panes"]
        .as_array()
        .unwrap()
        .iter()
        .find(|candidate| candidate["pane_id"] == pane.parse::<u64>().unwrap())
        .unwrap()["agent"];
    assert_eq!(listed_agent["session_present"], true);
    assert!(listed_agent["agent_session"].is_null());
    assert!(
        !serde_json::to_string(&listed)
            .unwrap()
            .contains("conversation-screen-1")
    );

    // State and session reports share one source clock.
    assert_eq!(
        error_code(command(
            &runtime,
            &name,
