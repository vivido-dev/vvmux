...
VVWS/1 remains protocol version 1. Its server-side session adapter uses private VVMX version 20,
...
mode flags into native host terminals. VVMX 18 reports recreated retained tracks so the session
adapter can restore image/raster bodies after a tab switch. Media snapshots preserve live/timed
plus active-slot state for relayed tracks. It remains a hard cutover: mixed client/server versions
are rejected.
